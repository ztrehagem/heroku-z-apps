...
module.exports = class Room {
  ...
  static index() {
    ...
    return redis.keysAsync(keyPattern)
      .then(keys => keys.map(key => new Room(keyToToken(key), redis)))
      .then(rooms => rooms.map(room => room.fetchSummary(true).catch(()=> null)))
  ...
  //   const keys = names.map(name => this[`${name}Key`]);
  //   return this.redis.watchAsync(keys);
  // }
  watch(keepAlive, fn) {
    if (typeof keepAlive == 'function') [keepAlive, fn] = [null, keepAlive];

    if (!this.connected) this.connect();

    const proc = (tryCount)=> {
      return Promise.resolve(fn(new WatchedRoom(this.token, this.redis)))
        .execAsync()
        .then(result => {
          return result || (tryCount >= 3 ? Promise.reject() : proc(tryCount + 1));
        });
    };
  ...
  fetchSummary(keepAlive) {
    if (!this.connected) this.connect();

    return this.redis.hgetallAsync(this.summaryKey)
  ...
  fetchField(keepAlive) {
    if (!this.connected) this.connect();

    return this.redis.lrangeAsync(this.fieldKey, 0, -1)
...
class WatchedRoom extends Room {
  fetchSummary() {
    this.redis.watch
    return super.fetchSummary(true,);
  }
  fetchField() {
